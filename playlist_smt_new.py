...
def build_and_solve(
    ...
    for j in range(max_slots):
        ...
        opt.add(Sum(If(x[i][j], 1, 0) for i in range(n)) <= 1)
    # ------------------------------------------------------------------
    ...
    track_used = [Int(f"u_{j}") for j in range(max_slots)]
    for j in range(max_slots):
        ...
        opt.add(track_used[j] == Sum(If(x[i][j], 1, 0) for i in range(n)))  # 0 or 1
    ...
    for j in range(max_slots - 1):
        both = And(track_used[j] == 1, track_used[j+1] == 1)
        # Only define jumps if both slots have a track
        opt.add(Implies(both, zv[j] >=  valence_j[j+1] - valence_j[j]))
        opt.add(Implies(both, zv[j] >=  valence_j[j]   - valence_j[j+1]))
        opt.add(Implies(both, zv[j] <=  val_delta))
        opt.add(Implies(Not(both), zv[j] == 0))

        opt.add(Implies(both, ze[j] >=  energy_j[j + 1] - energy_j[j]))
        opt.add(Implies(both, ze[j] >=  energy_j[j]     - energy_j[j + 1]))
        opt.add(Implies(both, ze[j] <=  energy_delta))
        opt.add(Implies(Not(both), ze[j] == 0))

    # ------------------------------------------------------------------
    # 5.  Average popularity (optional)
    # ------------------------------------------------------------------
    if pop_avg > 0:
        total_used = Sum(track_used)                      # real
        pop_sum    = Sum(tracks[i]["popularity"] * If(x[i][j],1,0)
                        for i in range(n) for j in range(max_slots))

        # Only add the constraint when at least one real track is chosen
        opt.add(Implies(total_used >= 1,
                        pop_sum >= pop_avg * total_used))
    ...
    # ------------------------------------------------------------------
    roughness = Sum(zv + ze)
    length_penalty = Sum(track_used) * 0.05 
    ...
    opt.minimize(roughness + length_penalty)
    ...
    check = opt.check()
    if check== sat or check == unknown:
        m = opt.model()
        # print("✓ Z3 returned:", check)
    # check = opt.check()
